...
export default function VastuShastraPage() {
  const { vastuConfig } = useAdminStore();
  const { hero } = vastuConfig;

  return (
    <>
      <section className="relative min-h-[50vh] flex items-center overflow-hidden bg-brand-dark">
        <div className="absolute inset-0">
          <img src={hero.bgImage} alt="" className="w-full h-full object-cover opacity-30" />
        </div>
      ...
      <VastuShastraGuide data={vastuConfig} />
    </>
  );
}
