...
export function ServiceDetailInner({ detail, bundle }: Props) {
  ...
  return (
    <>
      ...
      {detail.blog_categories?.map((cat) => {
        const posts = (bundle.blog_categories?.[cat.slug] ?? []).slice(0, 4);
        return (
            ...
            {posts.length === 0 ? (
              ...
              <section className="bg-white py-12 sm:py-16">
                <div className="max-w-[1160px] mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <h2 className="font-display font-bold text-[#0f2557] text-2xl mb-6">{cat.name}</h2>
      ...
      {detail.project_categories?.map((cat) => {
        const projects = (bundle.projects_by_category?.[cat.slug] ?? []).slice(0, 4);
        return (
