  ...
  return (
    <div className="rounded-lg border border-[#e8edf5] bg-white overflow-hidden">
      <div className="border-b border-[#e8edf5] px-5 py-3">
        <p className="text-[11px] font-bold uppercase tracking-[.07em] text-[#0f2557]">Room-Specific Vastu</p>
      </div>
      <div className="px-5 py-4">
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={loading}
            className="flex h-10 items-center gap-1.5 rounded-lg bg-[#cd2028] px-4 text-xs font-bold text-white hover:bg-[#b01c22] transition shrink-0 disabled:opacity-50"
          >
        ...
        {result && (
          <div className="mt-5 space-y-4">
            {(result.ideal_direction?.en || result.facing_direction?.en) && (
              <div className="rounded-lg border border-[#e8edf5] bg-[#f8fafd] px-4 py-3 space-y-2">
                {result.ideal_direction?.en && (
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[.1em] text-[#0f2557] mb-0.5">Ideal Direction</p>
                    <p className="text-xs text-[#3d526e]">{result.ideal_direction.en}</p>
                    {result.ideal_direction.np && (
                      <p className="text-[10px] text-[#5a6e8a] mt-0.5 border-l-2 border-[#0f2557]/20 pl-2">{result.ideal_direction.np}</p>
                    )}
                  </div>
                )}
                {result.facing_direction?.en && (
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-[.1em] text-[#cd2028] mb-0.5">Facing Direction</p>
                    <p className="text-xs text-[#3d526e]">{result.facing_direction.en}</p>
                    {result.facing_direction.np && (
                      <p className="text-[10px] text-[#5a6e8a] mt-0.5 border-l-2 border-[#cd2028]/20 pl-2">{result.facing_direction.np}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            {result.benefits && result.benefits.length > 0 && (
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[.1em] text-green-700 mb-2">Do&apos;s</p>
            ...
            {result.avoids && result.avoids.length > 0 && (
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[.1em] text-red-600 mb-2">Don&apos;ts</p>
                <div className="space-y-1.5">
